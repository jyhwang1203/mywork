...
class GridTradingBot:
    ...
    def exit_all(self, reason):
        ...
        # ── 매도 주문은 유지 (거래소에서 계속 실행됨) ──
        active_sells = sum(1 for lv in self.levels.values()
                           if lv['state'] == self.FILLED and lv.get('sell_uuid'))
        if active_sells > 0:
            log(f"  ✅ 매도 주문 {active_sells}건 유지 (거래소에서 계속 실행)")

        # ── 보유 코인 현황 ──
        total_holding_qty = sum(lv['buy_qty'] for lv in self.levels.values()
                                if lv['state'] == self.FILLED)
        total_holding_value = total_holding_qty * self.current_price

        unrealized_profit = sum(
            (self.current_price - lv['buy_price']) * lv['buy_qty']
            for lv in self.levels.values()
            if lv['state'] == self.FILLED
        )
        ...
        if total_holding_qty > 0:
            log(f"  💎 보유 코인 유지: {total_holding_qty:.6f}개 (≈{format_price(total_holding_value)}원)")
            log(f"  💰 평가 손익: {unrealized_profit:+,.0f}원")
        if self.profit_coins > 0:
            log(f"  💎 수익 코인: {self.profit_coins:.6f}개")
        ...
        for lv in self.levels.values():
            if lv['state'] == self.BUY_PENDING:
                lv['state'] = self.EMPTY
            lv['buy_uuid'] = None
            # sell_uuid 는 유지 (실제 주문이 살아있으므로)

        runtime = str(datetime.now() - self.start_time).split('.')[0]
        log(f"⏹️ 익절 완료")
        log(f"   매수 취소: {cancelled_buy}건 | 매도 유지: {active_sells}건 | 보유: {total_holding_qty:.6f}개")
