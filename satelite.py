...
# -*- coding: utf-8 -*-
"""
Upbit Grid Trading Bot V8.3
━━━━━━━━━━━━━━━━━━━━━━━━━━━
V8.3 변경: 스프레드% 기반 그리드 자동 설정
  - SPREAD_PCT(칸당 최소 수익률%)로 그리드 간격 결정
  - 등비수열(기하 간격)으로 레벨 생성 → 어느 가격대나 동일한 % 스프레드
  - 손익분기 공식: 1/(1-f)²-1 ≈ 0.1001% (수수료 0.05% 왕복 기준)
  - GRID_COUNT는 최대 칸수 안전 상한으로만 사용
"""
...
# ============================================================================
API_CONFIG = {
    'ACCESS_KEY': 'UYONkysbeJvn144jV0jcW7PxIE83YL8Ory3mrSdB',
    'SECRET_KEY': 'EZkIlOJXxV6LPgZ5mlNA71KCd0S09gUt8JjHRC4n',
}
...
CONFIG = {
    ...
    # ── 스프레드 기반 그리드 설정 (V8.3 핵심) ──────────────────────
    # 손익분기: 1/(1-0.0005)^2 - 1 = 0.1001%
    # 권장 최소: 0.15% (슬리피지 여유), 안정형: 0.20~0.50%
    'SPREAD_PCT': 0.002,      # 칸당 목표 스프레드 (0.002 = 0.20%)
    'SPREAD_MODE': 'GEOMETRIC',  # GEOMETRIC(등비, 권장) | LINEAR(등차)
    ...
    # ── 동적 매수 비율 (잔여현금 대비) ──────────────────────────────
    'DYNAMIC_BUY_RATIO': 0.10,   # 잔여현금의 10% = 1/10
}
...
def breakeven_spread(fee_pct=None):
    """왕복 수수료 기준 손익분기 최소 스프레드 반환"""
    f = fee_pct if fee_pct is not None else CONFIG['FEE_PCT']
    return 1 / ((1 - f) ** 2) - 1


def calc_grid_levels_geometric(lower, upper, spread_pct, max_count=200):
    """
    등비수열 그리드 레벨 생성.
    level[i+1] = level[i] * (1 + spread_pct)
    틱 반올림 후 중복/역전 제거.
    반환: 정렬된 레벨 리스트 (len >= 2)
    """
    if spread_pct <= 0 or lower >= upper:
        return []

    levels = [lower]
    current = lower
    while current < upper and len(levels) <= max_count:
        nxt = round_to_tick(current * (1 + spread_pct))
        if nxt <= current:
            nxt = current + get_tick_size(current)
        if nxt > upper:
            break
        levels.append(nxt)
        current = nxt

    # 마지막 레벨이 upper에 못 미치면 upper 추가
    if levels[-1] < upper:
        upper_tick = round_to_tick(upper, 'up')
        if upper_tick > levels[-1]:
            levels.append(upper_tick)

    return levels


def calc_grid_levels_linear(lower, upper, spread_pct, max_count=200):
    """
    등차수열 그리드 레벨 생성.
    step = lower * spread_pct (고정 원화 간격)
    틱 반올림 후 중복/역전 제거.
    """
    if spread_pct <= 0 or lower >= upper:
        return []

    step_raw = lower * spread_pct
    tick = get_tick_size(lower)
    step = max(round_to_tick(step_raw), tick)

    levels = [lower]
    current = lower
    while current < upper and len(levels) <= max_count:
        nxt = round_to_tick(current + step)
        if nxt <= current:
            nxt = current + tick
        if nxt > upper:
            break
...
class TelegramNotifier:
    ...
    def notify_start(self, data):
        ...
        msg = (
            ...
            f"📐 스프레드: {data.get('spread_pct', 0) * 100:.4f}% ({data.get('spread_mode', 'GEOMETRIC')}) | {data.get('grid_count', 0)}칸\n"
            f"💹 매도전략: {sell_mode_str}\n"
            f"📝 사유: {data['reason']}\n"
            f"💡 그리드 매수: 잔여현금 × {CONFIG['DYNAMIC_BUY_RATIO'] * 100:.1f}% (동적)\n"
            f"━━━━━━━━━━━━━━\n"
        )
...
def manual_select_target():
    ...
    # ── 스프레드 설정 ─────────────────────────────────────────────
    be = breakeven_spread() * 100   # 손익분기 %
    print(f"\n{'=' * 55}")
    print(f"📐 칸당 스프레드(%)를 입력하세요")
    print(f"   손익분기: {be:.4f}% (수수료 0.05% 왕복)")
    print(f"   권장 최소: 0.15%  |  안정형: 0.20~0.50%")
    print(f"   ※ 스프레드 클수록 칸수↓, 수익/칸↑, 체결 빈도↓")
    print(f"{'=' * 55}")

    spread_pct = None
    while True:
        try:
            s_input = input(f"\n스프레드 입력 (예: 0.2 = 0.20%): ").strip()
            s_val = float(s_input) / 100
            if s_val <= 0:
                print("❌ 0보다 큰 값을 입력해주세요.")
                continue
            if s_val < breakeven_spread():
                print(f"⚠️  입력값({s_val * 100:.4f}%)이 손익분기({be:.4f}%) 미만입니다.")
                confirm_be = input("   그래도 계속하시겠습니까? (y/n): ").strip().lower()
                if confirm_be != 'y':
                    continue
            spread_pct = s_val
            break
    ...
    # ── 스프레드 방식 ──────────────────────────────────────────────
    print("\n간격 방식:")
    print("  1) 등비(GEOMETRIC) - 어느 가격대나 동일 % 간격 (권장)")
    print("  2) 등차(LINEAR)    - 고정 원화 간격 (저가 코인 적합)")
    while True:
        sm = input("\n선택 (1 또는 2, 기본=1): ").strip()
        if sm in ('', '1'):
            CONFIG['SPREAD_MODE'] = 'GEOMETRIC'
            print("✅ 등비 간격")
            break
        elif sm == '2':
            CONFIG['SPREAD_MODE'] = 'LINEAR'
            print("✅ 등차 간격")
            break
        else:
    ...
    # ── 예상 칸수 미리보기 ────────────────────────────────────────
    lower_tick = round_to_tick(lower, 'down')
    upper_tick = round_to_tick(upper, 'up')
    if CONFIG['SPREAD_MODE'] == 'GEOMETRIC':
        preview_levels = calc_grid_levels_geometric(lower_tick, upper_tick, spread_pct, max_count=CONFIG['GRID_COUNT'])
    else:
        preview_levels = calc_grid_levels_linear(lower_tick, upper_tick, spread_pct, max_count=CONFIG['GRID_COUNT'])

    preview_grids = max(0, len(preview_levels) - 1)
    if preview_grids > 0:
        actual_spreads = [(preview_levels[i+1] - preview_levels[i]) / preview_levels[i] * 100
                          for i in range(len(preview_levels) - 1)]
        avg_spread = sum(actual_spreads) / len(actual_spreads)
        min_spread = min(actual_spreads)
        print(f"\n📋 예상 그리드: {preview_grids}칸")
        print(f"   평균 스프레드: {avg_spread:.4f}% | 최소: {min_spread:.4f}%")
        print(f"   첫 칸: {format_price(preview_levels[0])} → {format_price(preview_levels[1])}원")
        if preview_grids > 1:
            print(f"   마지막: {format_price(preview_levels[-2])} → {format_price(preview_levels[-1])}원")
    else:
        print(f"\n⚠️  스프레드가 너무 커서 그리드가 생성되지 않습니다. 값을 줄여주세요.")
...
class GridTradingBot:
    ...
    def __init__(self, target, grid_amount, grid_count, available_cash=None):
        ...
        self.grid_unit = grid_amount        # 초기 매수 기준금액
        self.grid_count = grid_count        # 최대 칸수 상한
        # capital은 build_grid() 후 실제 칸수 기반으로 재계산됨
        ...
        # ── 스프레드 기반 그리드 설정 (V8.3) ──────────────────────
        self.spread_pct  = target.get('spread_pct', CONFIG.get('SPREAD_PCT', 0.002))
        self.spread_mode = CONFIG.get('SPREAD_MODE', 'GEOMETRIC')
        be = breakeven_spread()
        log(f"📐 스프레드: {self.spread_pct * 100:.4f}% ({self.spread_mode}) "
            f"| 손익분기: {be * 100:.4f}% | 여유: {(self.spread_pct - be) * 100:.4f}%")

        # ── 동적 매수 비율 ─────────────────────────────────────────
        self.dynamic_buy_ratio = CONFIG.get('DYNAMIC_BUY_RATIO', 0.10)
        log(f"💡 동적 매수: 잔여현금 × {self.dynamic_buy_ratio * 100:.1f}%")
        ...
        if self.stop_loss_price > 0:
            log(f"🔻 손절가 설정: {format_price(self.stop_loss_price)}원")
        if self.hard_take_profit_price > 0:
            log(f"🔺 강제 익절가 설정: {format_price(self.hard_take_profit_price)}원")
        ...
        self.range_pct = target.get('range_pct', 0.05)

    # ════════════════════════════════════════════════════════════
    # 동적 매수 금액 계산 (핵심 변경 포인트)
    # ════════════════════════════════════════════════════════════
    def _calc_dynamic_unit(self):
        """
        잔여 현금의 DYNAMIC_BUY_RATIO(기본 10%)를 한 그리드 매수 금액으로 반환.
        MIN_ORDER_AMOUNT 보다 작으면 None 반환 (잔고 부족).
        """
        unit = self.cash * self.dynamic_buy_ratio
        if unit < CONFIG['MIN_ORDER_AMOUNT']:
            return None
        # MAX_ORDER_AMOUNT 초과 방지
        return min(unit, CONFIG['MAX_ORDER_AMOUNT'])

    # ════════════════════════════════════════════════════════════
    # STEP 1: 초기 지정가 매수 (GRID_AMOUNT 1회 사용)
    # ════════════════════════════════════════════════════════════
    def execute_initial_buy(self):
        ...
        if ticker and isinstance(ticker, list) and len(ticker) > 0:
            ...
            log(f"📡 최신 시세: {format_price(self.current_price)}원")
        else:
    ...
    def build_grid(self):
        ...
        # ── 스프레드 기반 레벨 생성 (V8.3 핵심) ────────────────────
        spread_pct   = self.spread_pct
        spread_mode  = self.spread_mode
        max_count    = self.grid_count  # 안전 상한

        be = breakeven_spread()
        if spread_pct < be:
            log(f"  ⚠️ 설정 스프레드({spread_pct * 100:.4f}%) < 손익분기({be * 100:.4f}%) — 손실 위험", 'WARN')

        if spread_mode == 'LINEAR':
            final_levels = calc_grid_levels_linear(initial_lower, initial_upper, spread_pct, max_count)
            mode_label = '등차(LINEAR)'
        else:
            final_levels = calc_grid_levels_geometric(initial_lower, initial_upper, spread_pct, max_count)
            mode_label = '등비(GEOMETRIC)'

        if len(final_levels) < 2:
            # 스프레드가 너무 커서 레벨이 생성 안 됨 → 최소 2칸 강제 생성
            tick = get_tick_size(initial_lower)
            mid = round_to_tick(initial_lower * (1 + spread_pct))
            if mid <= initial_lower:
                mid = initial_lower + tick
            final_levels = [initial_lower, mid, initial_upper]
            log(f"  ⚠️ 레벨 생성 실패 → 최소 2칸 강제 생성", 'WARN')

        actual_count = len(final_levels) - 1

        if actual_count < 1:
            log(f"❌ 그리드 수 부족 ({actual_count}칸) — 범위 또는 스프레드를 확인하세요", 'ERROR')
            return False

        if actual_count >= max_count:
            log(f"⚠️ 최대 칸수 상한({max_count}) 도달 → {actual_count}칸으로 제한", 'WARN')

        # ── 실제 스프레드 통계 ───────────────────────────────────
        actual_spreads = [(final_levels[i+1] - final_levels[i]) / final_levels[i] * 100
                          for i in range(len(final_levels) - 1)]
        avg_sp = sum(actual_spreads) / len(actual_spreads)
        min_sp = min(actual_spreads)
        max_sp = max(actual_spreads)
        log(f"  ✅ {mode_label} | {actual_count}칸 생성")
        log(f"     스프레드: 평균 {avg_sp:.4f}% | 최소 {min_sp:.4f}% | 최대 {max_sp:.4f}%")
        log(f"     범위: {format_price(final_levels[0])} ~ {format_price(final_levels[-1])}원")

        self.grid_levels = final_levels
        self.actual_grid_count = actual_count

        # capital을 실제 칸수 기반으로 재계산 (동적 매수이므로 참고용)
        self.capital = self.grid_unit * actual_count
    ...
    def place_initial_orders(self):
        ...
        lv['buy_amount'] = self.initial_cost   # 초기 매수 비용 기록
    ...
    # ════════════════════════════════════════════════════════════
    # 계단식 매수 — 잔여현금 × 1/10 동적 금액 적용 (핵심 변경)
    # ════════════════════════════════════════════════════════════
    def _fill_buys_below_price(self):
        ...
        for idx, price in current_buy_orders:
            ...
            if need_cancel:
                ...
                # ▼ 실제 투입 금액 복원 (동적 금액이므로 buy_amount 사용)
                self.cash += lv.get('buy_amount', self.grid_unit)
                lv['state'] = self.EMPTY
                lv['buy_uuid'] = None
                lv['buy_qty'] = 0
                lv['buy_amount'] = 0.0
        ...
        for idx, buy_price in candidates:
            ...
            # ── 동적 매수 금액: 잔여현금 × DYNAMIC_BUY_RATIO ─────────
            unit = self._calc_dynamic_unit()
            if unit is None:
                skipped_low_cash += 1
                log(f"  💰 Lv.{idx} 스킵: 잔여현금({format_price(self.cash)}원) × "
                    f"{self.dynamic_buy_ratio * 100:.1f}% = {format_price(self.cash * self.dynamic_buy_ratio)}원 "
                    f"< 최소주문({CONFIG['MIN_ORDER_AMOUNT']}원)")
                continue
            ...
            if res and 'uuid' in res:
                ...
                lv['buy_qty'] = qty
                lv['buy_amount'] = unit          # ▼ 실제 투입 금액 저장
                self.cash -= unit
                placed += 1
                log(f"  🟢 Lv.{idx} 매수 배치: {format_price(price)}원 × {qty:.6f} "
                    f"(투입: {format_price(unit)}원 | 잔금: {format_price(self.cash)}원)")
        ...
        for i, lv in list(self.levels.items()):
            if lv['state'] == self.BUY_PENDING and lv['buy_uuid']:
                ...
                if order.get('state') == 'done':
                    ...
                    # buy_amount는 주문 시점에 이미 차감됐으므로 유지
                    lv['state'] = self.FILLED
                    lv['buy_uuid'] = None
                    self.trade_count += 1
                    log(f"🟢 체결 Lv.{i}: {format_price(lv['buy_price'])}원 × {lv['buy_qty']:.6f} "
                        f"(투입: {format_price(lv.get('buy_amount', 0))}원 | 잔금: {format_price(self.cash)}원)")
                    self.notifier.notify_trade('BUY', self.market, lv['buy_price'], lv['buy_qty'])
                    self._place_sell(i)
                    self._refresh_avg_sells()
    ...
    def get_total_value(self):
        ...
        # ▼ 대기 주문 평가: 실제 투입 금액(buy_amount) 사용
        pending_val = sum(lv.get('buy_amount', self.grid_unit)
    ...
    def print_status(self):
        ...
        # 다음 그리드 매수 예정 금액 표시
        next_unit = self._calc_dynamic_unit()
        next_unit_str = format_price(next_unit) if next_unit else "잔고부족"

        print(f"\n{'─' * 55}")
        print(f"💡 다음 그리드 매수 예정: {next_unit_str}원 "
              f"(잔금 {format_price(self.cash)}원 × {self.dynamic_buy_ratio * 100:.1f}%)")
        ...
        for i in range(len(self.levels) - 1, -1, -1):
            ...
            if st == self.FILLED:
                ...
                # 투입 금액 표시
                amt = lv.get('buy_amount', 0)
                sym = f"🟩({format_price(amt)}원)"
            else:
                sym = "⬜"
    ...
    def _print_grid(self):
        ...
        if len(self.grid_levels) >= 2:
            spreads = [(self.grid_levels[i+1] - self.grid_levels[i]) / self.grid_levels[i] * 100
        ...
        print(f"\n{'=' * 60}")
        print(f"🎯 Grid Bot V8.3 | {'실전' if CONFIG['ENABLE_REAL_TRADING'] else '시뮬레이션'}")
        print(f"{'=' * 60}")
        ...
        print(f"  📐 {actual_grids}칸 | {self.spread_mode}")
        print(f"     목표 스프레드: {self.spread_pct * 100:.4f}%")
        print(f"     실제 평균:     {avg_sp:.4f}% | 최소: {min_sp:.4f}%")
        print(f"     손익분기:      {be * 100:.4f}% | 여유: {(min_sp/100 - be) * 100:.4f}%")
        print(f"  💡 그리드 매수: 잔여현금 × {self.dynamic_buy_ratio * 100:.1f}%"
              f" (현재 ≈{format_price(self.cash * self.dynamic_buy_ratio)}원)")
        ...
        for i, p in enumerate(self.grid_levels):
            if i < len(self.grid_levels) - 1:
                sp = (self.grid_levels[i+1] - p) / p * 100
                sp_warn = " ⚠" if sp / 100 < be else ""
                tag = " ◀ 진입" if i > 0 and self.grid_levels[i-1] <= self.entry_price < self.grid_levels[i] else ""
                print(f"  Lv.{i:>2d}: {format_price(p):>12s}원  +{sp:.4f}%{sp_warn}{tag}")
            else:
                tag = " ◀ 진입" if self.grid_levels[i-1] <= self.entry_price < p else ""
                print(f"  Lv.{i:>2d}: {format_price(p):>12s}원  (상단){tag}")
        print(f"{'=' * 60}\n")
...
def run_bot_cycle(target, grid_amount, grid_count):
    ...
    if CONFIG['ENABLE_REAL_TRADING']:
        ...
        if krw_balance:
            ...
            log(f"   첫 그리드 매수 예정: {format_price(available_cash * CONFIG['DYNAMIC_BUY_RATIO'])}원 "
                f"(잔고 × {CONFIG['DYNAMIC_BUY_RATIO'] * 100:.0f}%)")
        else:
    ...
    bot.notifier.notify_start({
        ...
        'spread_pct': bot.spread_pct,
        'spread_mode': bot.spread_mode,
    })
...
def main():
    ...
    print("=" * 55)
    print("💰 그리드 매수 비율 설정")
    print("   매수 주문 시: 그 시점 잔여현금 × N% 를 한 칸에 투입")
    print(f"   현재 기본값: {CONFIG['DYNAMIC_BUY_RATIO'] * 100:.1f}%")
    print("=" * 55)
    while True:
        try:
            ratio_input = input("\n매수 비율을 입력하세요 (예: 10 = 10%, 기본=10): ").strip()
            if ratio_input == '':
                ratio_input = '10'
            ratio_val = float(ratio_input)
            if ratio_val <= 0 or ratio_val > 100:
                print("❌ 0%보다 크고 100% 이하로 입력해주세요.")
                continue
            CONFIG['DYNAMIC_BUY_RATIO'] = ratio_val / 100
            print(f"✅ 매수 비율: {ratio_val:.1f}% (잔여현금의 {ratio_val:.1f}%씩 매수)")
            # 이론 상 몇 번 매수 가능한지 참고 표시
            approx_buys = int(1 / CONFIG['DYNAMIC_BUY_RATIO'])
            print(f"   → 이론상 약 {approx_buys}회 매수 후 잔고 소진 (매수마다 비율 재적용)")
            break
        except ValueError:
            print("❌ 숫자를 입력해주세요.")
    ...
    log(f"모드: {mode}")
    log(f"초기 매수금액: {CONFIG['GRID_AMOUNT']:,.0f}원 | 최대 칸수 상한: {CONFIG['GRID_COUNT']}칸")
    log(f"스프레드: {CONFIG['SPREAD_PCT'] * 100:.4f}% ({CONFIG['SPREAD_MODE']}) "
        f"| 손익분기: {be * 100:.4f}% | 여유: {(CONFIG['SPREAD_PCT'] - be) * 100:.4f}%")
    log(f"그리드 매수: 잔여현금 × {CONFIG['DYNAMIC_BUY_RATIO'] * 100:.1f}% (사용자 설정, 동적)")
    ...
    try:
        while True:
            cycle += 1
            log(f"\n{'═' * 55}")
            log(f"🔄 사이클 #{cycle} | 누적손익: {session_profit:+,.0f}원")
            log(f"{'═' * 55}")
