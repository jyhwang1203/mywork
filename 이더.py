...
# -*- coding: utf-8 -*-
"""
Upbit Grid Trading Bot V8.2 (패치: 그리드당 매수금액 = 잔여현금 × 1/10)
"""
...
CONFIG = {
    ...
    'GRID_AMOUNT': 10000.0,   # 초기 매수(1회) 에만 사용; 이후 그리드 매수는 잔여현금 × 1/10
    'GRID_COUNT': 40,
    'MAX_BUY_ORDERS': 3,
...
class TelegramNotifier:
    ...
    def notify_start(self, data):
        ...
        msg = (
            ...
            f"📝 사유: {data['reason']}\n"
            f"💡 그리드 매수: 잔여현금 × {CONFIG['DYNAMIC_BUY_RATIO'] * 100:.0f}% (동적)\n"
            f"━━━━━━━━━━━━━━\n"
        )
...
class GridTradingBot:
    ...
    def __init__(self, target, grid_amount, grid_count, available_cash=None):
        ...
        self.grid_unit = grid_amount        # 초기 매수 금액 기준 (그리드 평가 기준)
        ...
        log(f"💡 동적 매수 활성화: 그리드 주문 시 잔여현금 × {self.dynamic_buy_ratio * 100:.0f}%")
        ...
        for idx, buy_price in candidates:
            ...
            if unit is None:
                ...
                log(f"  💰 Lv.{idx} 스킵: 잔여현금({format_price(self.cash)}원) × "
                    f"{self.dynamic_buy_ratio * 100:.0f}% = {format_price(self.cash * self.dynamic_buy_ratio)}원 "
    ...
    def print_status(self):
        ...
        print(f"💡 다음 그리드 매수 예정: {next_unit_str}원 "
              f"(잔금 {format_price(self.cash)}원 × {self.dynamic_buy_ratio * 100:.0f}%)")
    ...
    def _print_grid(self):
        ...
        print(f"  💡 그리드 매수: 잔여현금 × {self.dynamic_buy_ratio * 100:.0f}% (동적, 첫 매수 기준 ≈{format_price(self.cash * self.dynamic_buy_ratio)}원)")
        ...
        if actual_grids < target_grids:
            print(f"  📐 {actual_grids}칸 (목표: {target_grids}칸) | 총 자본: {format_price(self.capital)}원")
        else:
            print(f"  📐 {actual_grids}칸 | 총 자본: {format_price(self.capital)}원")
...
def main():
    ...
    log(f"모드: {mode}")
    log(f"초기 매수금액 기준: {CONFIG['GRID_AMOUNT']:,.0f}원 × {CONFIG['GRID_COUNT']}칸 = {total_capital:,.0f}원")
    log(f"그리드 매수 방식: 잔여현금 × {CONFIG['DYNAMIC_BUY_RATIO'] * 100:.0f}% (동적)")
